...
class ButtonContributionManager {
  ...
  private isActive(contrib: ButtonContributionParams) {
    ...
    const parent = document.querySelector(contrib.selector);
    if (parent === null) {
      return false;
    }
  ...
  private installAnchor(contrib: ButtonContributionParams) {
    ...
    const before = contrib.insertBefore && document.querySelector(contrib.insertBefore);

    if (contrib.manipulations) {
      for (const manipulation of contrib.manipulations) {
        const element = document.querySelector(manipulation.element);
        if (element) {
